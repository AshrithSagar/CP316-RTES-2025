...
#define GPIO_OUTSET                                                            \
  0x508 // set pins as per the mask
        //   1 sets the pin, 0 no effect
#define GPIO_OUTCLR                                                            \
  0x50c               // clear pins as per the mask
                      //   1 clears the pin, 0 no effect
#define GPIO_IN 0x510 // read a port
#define GPIO_DIR                                                               \
  0x514 // set directions of all 32 pins of a port
        //   0 input, 1 output
...
void gpio_inten(uint32_t pin, uint32_t event_no, uint32_t edge,
                pfn_t callback) {
   ...
   */
  GPIOTE_CONFIG(event_no) = ((GPIOTE_MODEEVENT) | (pin << 8) | (edge << 16));
